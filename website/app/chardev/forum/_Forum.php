<?php
	require_once 'forum/ForumPermissions.php';
	require_once 'forum/ThreadDatabase.php';
	require_once 'GameDatabase.php';
	...
	class _Forum {
		...
		public function reply( $threadId ) {
			...
			$threadLink = _Forum::threadLink( $thread );
			$posts = $this->database->getPosts( $threadId, 5, 0, false );
			$pages = (int)(($thread['PostCount']-1) / _Forum::POSTS_PER_PAGE + 1);
			$codes = _Forum::getAvailableForumCode();
		...
		private function getForum( $hook, $index ) {
			...
			if( $latestPost )
			{
				$latestPostUser = new User($latestPost['AuthorID']);
				$roleCss = _Forum::roleToCssClass($latestPostUser);
				$date = _Forum::timestampToString((int)$latestPost['Created']);
				$link = _Forum::threadLink($this->database->getThread($latestPost["ThreadID"]), 30, true);
				
				$html .="
					<span class=\"forum_time\">Posted {$date}
					by <span class=\"{$roleCss}\">{$latestPostUser->getName()}</span></span>
					<div>{$link}</div>
				";
			}
			$html .="
		...
		public function getThreads( $forumId, $page ) {
			...
			$pages = (int)(($hook['ThreadCount']-1) / _Forum::THREADS_PER_PAGE + 1);
			$page = max(min($pages,$page),1);
			$stickies = $this->database->getStickies($forumId,_Forum::THREADS_PER_PAGE, max(0,$page-1) * _Forum::THREADS_PER_PAGE);
			$announcements = $this->database->getAnnouncements($forumId,_Forum::THREADS_PER_PAGE, max(0,$page-1) * _Forum::THREADS_PER_PAGE);
			$threads = $this->database->getThreads($forumId,_Forum::THREADS_PER_PAGE, max(0,$page-1) * _Forum::THREADS_PER_PAGE);
			$actions = "";
		...
		private function getThread( $thread, $page, $index ) {
			$user = new User($thread['AuthorID']);
			$roleColorCssClass = _Forum::roleToCssClass($user);
			$threadCreated = _Forum::timestampToString((int)$thread['Created']);
			$threadLink = _Forum::threadLink( $thread );
			$pages = (int)(($thread['PostCount']-1) / _Forum::POSTS_PER_PAGE + 1);
			
			$userPost = new User($thread['PostAuthorID']);
			$roleColorCssClassPost = _Forum::roleToCssClass($userPost);
			$postCreated = _Forum::timestampToString((int)$thread['PostCreated']);
		...
		public function getPosts( $threadId, $page, $editPostId = 0 ) {
			...
			$pages = (int)(($thread['PostCount']-1) / _Forum::POSTS_PER_PAGE + 1);
			$page = max(min($pages,$page),1);
			$posts = $this->database->getPosts( $threadId, _Forum::POSTS_PER_PAGE, max(0,$page-1) * _Forum::POSTS_PER_PAGE );
			$hook = $this->database->getHook($thread['ThreadHookID']);
			$threadLink = _Forum::threadLink( $thread );
		...
		<table cellpadding='0' cellspacing='0' class='fo_table'>
			...
			for( $i=0; $i<count($posts); $i++ ) {
				$html .= $this->getPost( $posts[$i], $page, ($page-1) * _Forum::POSTS_PER_PAGE + $i + 1, $thread, $editPostId );
			}
		...
		private function getPost( $post, $page, $threadIndex, $thread, $editPostId = 0 ) {
			$user = new User($post['AuthorID']);
			$date = _Forum::timestampToString((int)$post['Created']);
			...
			if( $post['ModCount'] > 0 ) {
				...
				$mods = "<div class='fo_post_mods'>Modified {$times}"._Forum::timestampToString((int)$post['LastCreated'])."</div>";
			}
			...
			if( $editPostId == $post['ID'] ) {
				...
				$content = _Forum::replaceCode($post['Plain']);
			}
			/*
			if( $editPostId == 0 ) {
			...
			*/
			...
			$actions = "";
			if( $this->permissions->mayEditPostsBy($post['AuthorID'], $thread['ThreadHookID'])  && ($thread["Flag"]&ThreadDatabase::FLAG_LOCKED) == 0  ) {
		...
		public static function roleToCssClass( User $user ) {
			if( $user->isAdmin() ) {
		...
		{
			$title = Chardev_FormatHelper::shorten(htmlspecialchars_decode($thread['Title'],ENT_QUOTES),$maxLength);
			$prefix = "";
			$cssClass = "";
			...
			$add = "";
			if( $showLastPost) {
				$page = ceil(((int)$thread["PostCount"]) / _Forum::POSTS_PER_PAGE);
		...
		private static function replaceExternItemLink($match){
			return $match[0]." "._Forum::replaceItemLink($match);
		}
	
		private static function replaceItemLink($match){
			if($match[1]){
				
				$db = new GameDatabase(Database::ENGLISH);
				$data = $db->getItemData($match[1]);
