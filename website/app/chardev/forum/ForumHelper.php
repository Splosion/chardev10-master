...
class ForumHelper {
	...
	public static function getLatestThreadPost( \chardev\forum\ThreadDatabase $db, $hook) {
	...
	public static function roleToCssClass( \chardev\forum\facades\UserFacade $user) {
		if ($user->isAdmin ()) {
	...
	public static function getThreadLastPostLink( \chardev\forum\facades\ThreadFacade $thread, $maxLength) {
	...
	public static function getMaxPagesThread( $thread ) {
	...
	public static function validateArgs( \chardev\forum\ThreadDatabase $db, $forumAction, $threadAction = null) {
		...
		if( ! $parsedHookUrl ) {
			$this->_helper->getHelper('Redirector')->gotoRoute(array(),'forum');
		}
		if( $threadAction ) {
	...
	public static function getThreadUrl( \chardev\forum\facades\ThreadFacade $threadFacade, $page = 1) {
	...
	public static function getHookUrl( \chardev\forum\facades\HookFacade $hook, $page = 1 ) {
		return \TemplateHelper::getBasePath() . 'forum/' .
					FormatHelper::verboseUrl($hook->getId(), $hook->getName(), $page) . '.html';
	}
	
	public static function getThreadLink( \chardev\forum\facades\ThreadFacade $threadFacade, $page = 1) {
		return self::getGenericThreadLink($threadFacade, null, $page, 50);
	}
	
	private static function getGenericThreadLink( \chardev\forum\facades\ThreadFacade $threadFacade, $anchor, $page = 1, $length = 50 ) {
		$prefix = "";
		$cssClass = "";
	...
	public static function getUserUrl( \chardev\forum\facades\UserFacade $user ) {
		return \TemplateHelper::getBasePath() . 'user/' . FormatHelper::verboseUrl($user->getId(),$user->getName()) . '.html';
	}
	
	public static function getUserLink( \chardev\forum\facades\UserFacade $user ) {
	...
	public static function getAvatar( \chardev\forum\facades\UserFacade $author, $class = 'forum_avatar' ) {
	...
	public static function getPostAnchor( \chardev\forum\facades\PostFacade $post, $page, $index ) {
	...
	public static function getThreadCreateLink( \chardev\forum\facades\HookFacade $hook ) {
	...
	public static function getReplyLink( \chardev\forum\facades\ThreadFacade $thread ) {
	...
	public static function getDeleteThreadLink( \chardev\forum\facades\ThreadFacade $thread ) {
		return "<a class=\"button button_light fo_header_action\" href=\"javascript:Chardev.deleteThread({$thread->getId()})\" >Delete</a>";
	}
	
	public static function getLockThreadLink( \chardev\forum\facades\ThreadFacade $thread ) {
		if($thread->isLocked()) {
	...
	public static function getEditPostLink( \chardev\forum\facades\PostFacade $post ) {
		$url = \TemplateHelper::getBasePath() . 'forum/' .
	...
	public static function getDeletePostLink( \chardev\forum\facades\PostFacade $post ) {
		return "<a class=\"fo_post_header_action\" href=\"javascript:Chardev.deletePost({$post->getId()})\" >Delete</a>";
	}
	
	public static function getPageOfPost( \chardev\forum\facades\PostFacade $post ) {
