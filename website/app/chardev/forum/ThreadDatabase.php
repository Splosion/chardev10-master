...
class ThreadDatabase {
	...
	private $db;
	private $createThreadCooldown = 10;
	...
	public function getThread( $threadId ) {
		$stmt = $this->db->prepare("SELECT 
				...
				t.`ThreadHookID` as ThreadHookID,
	...
	public function getThreadCount( $hookId ) {
		$stmt = $this->db->prepare("SELECT count(*) as Count FROM `thread` WHERE `ThreadHookID`=? AND NOT `Flag`&?");
		$stmt->execute( array((int)$hookId, self::FLAG_DELETED | self::FLAG_THREAD_ANNOUNCEMENT | self::FLAG_THREAD_STICKY ) );
		$this->checkExecutedStatement($stmt);
		$countResult = $stmt->fetch();
	...
	public function getPostCount( $threadId ) {
		$stmt = $this->db->prepare("SELECT count(*) as Count FROM `thread` WHERE `ThreadHookID`=? AND NOT `Flag`&?");
		$stmt->execute( array((int)$threadId, self::FLAG_DELETED) );
		$this->checkExecutedStatement($stmt);
		$countResult = $stmt->fetch();
		return $countResult['Count'];
	}
	...
	private function checkThreadCooldown( $authorId ) {
		if( $this->createThreadCooldown > 0 ) {
			$stmt = $this->db->prepare("SELECT `ID` FROM `thread` WHERE `AuthorID` = ? AND `Created` > ?");
			$stmt->execute(array($authorId, time() - $this->createThreadCooldown));
			$this->checkExecutedStatement( $stmt);
