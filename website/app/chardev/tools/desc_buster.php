...
<?php
	...
	for( $l = 0; $l < 1; $l ++ ) //TODO: parse other languages
	{
		mysql_select_db("chardev_mop");
		...
		$stmt = mysql_query('SELECT *, s.`ID` AS ID FROM `Spell` s LEFT JOIN `SpellMisc` m ON s.`SpellMiscID` = m.`ID` LIMIT 0,1000000');
		while($result = mysql_fetch_assoc($stmt))
		{
			...
			*/;
			...
			if( $l_to_lang[$l] == 0 ) {
				mysql_query(
					"replace into chardev_mop_static.chardev_spellinfo ( SpellID, DescriptionEN, Scalable, ElixirMask ) values ( ".
					$result['ID'].", '".
				...
				mysql_query(
					"update chardev_mop_static.chardev_spellinfo set Description".
	...
	*/
	...
	function variable_replace($match){
		...
		if($spell_id){
			$spell_result = mysql_query("SELECT *, s.`ID` AS ID FROM `Spell` s INNER JOIN `SpellMisc` m ON s.`SpellMiscID` = m.`ID` WHERE s.`ID`=".$spell_id);
		...
		switch($effect){
			case "A":
				$record2 = mysql_query("SELECT r.Radius FROM spellradius r, spelleffect e WHERE e.spellid=".$spell['ID']." and r.ID = e.EffectSpellRadiusID and e.Index =".($effect_nr-1));
	...
	function getEffect( $spell, $effect_nr, $prefix, $mod ) {
		...
		$n = ""; //TODO: Why is there only one coefficient in the db
		
		if( $GLOBALS['debug'] ) echo $spell['ID'].' '.$n.' '.(int)$spell['SpellScalingID'] .' '. (float)$effectRecord['Coefficient']."\n";
		if( (int)$spell['SpellScalingID'] != 0  ) {
			...
			if( ( $prefix == '$s' || $prefix == '$m' || $prefix == '$M' ) && (float)$effectRecord['Coefficient'] != 0 ) {
